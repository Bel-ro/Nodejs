...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write('Bienvenidos a DH Movies el mejor sitio para encontrar las mejroes peliculas, incluso mucho mejor que Netflix, Cuevana y PopCorn \n');
			let cantidadPeliculas = movies.length;
			res.write('Total de peliculas en cartelera: ' + cantidadPeliculas); 

			res.write('\n');
			res.write('Todas nuestras peliculas');

			let titulosPeliculas =  []

			movies.forEach(function(pelicula) {
				titulosPeliculas.push(pelicula.title)
			});
			res.write('\n')
				titulosPeliculas = titulosPeliculas.sort();
				for(let pelicula of titulosPeliculas){
					res.write('\n')
					res.write(`.${pelicula}`);
		...
		case '/mas-votadas':
			...
			res.write('Total de peliculas en cartelera: ' + masVotadas.length); 
			res.write('\n');
		...
		case '/sucursales':
			...
			theaters.forEach(function(theaters){
				...
				res.write(`\n`);
				res.write(`\n`);
				res.write(`Descricion: ${theaters.description}`);
			})
		...
		case '/contacto':
			...
			res.write(`\n`);
			res.write('​¿Tenés algo para contarnos? Nos encanta escuchar a nuestrosclientes. Si deseas contactarnos podés escribirnos al siguiente email:dhmovies@digitalhouse.com o en las redes sociales. Envianos tu consulta,sugerencia o reclamo y será respondido a la brevedad posible. Recordá quetambién podes consultar la sección de Preguntas Frecuentes para obtenerrespuestas inmediatas a los problemas más comunes.');
           res.end();
			break;
		case '/preguntas-frecuentes':
           
			res.write('Preguntas Frecuentes\n');
			res.write(`Total de preguntas frecuentes: ${faqs.length}`);
			res.write(`\n`);
			res.write(`\n`);
